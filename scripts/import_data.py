...
for mfr in df_tovar["Производитель"].unique():
    cursor.execute(
        "INSERT OR IGNORE INTO Manufacturers (manufacturer_name) VALUES (?)", (mfr,)
    )
...
for _, row in df_tovar.iterrows():
    ...
    manufacturer_id = get_id(
        "Manufacturers", "manufacturer_id", "manufacturer_name", row["Производитель"]
    )
    category_id = get_id(
        "Categories", "category_id", "category_name", row["Категория товара"]
    )
...
for _, row in df_orders.iterrows():
    ...
    """,
        (
            ...
            delivery_date,
            pickup_point_id,
            get_client_id(row["ФИО авторизированного клиента"]),
