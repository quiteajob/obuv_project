...
from database import (
    get_all_products, search_products, get_unique_suppliers,
    delete_product, get_next_product_id
)
from styles import (
    COLOR_MAIN_BG, COLOR_ACCENT_BG, COLOR_ACTION, 
    COLOR_DISCOUNT_BG, COLOR_OUT_OF_STOCK, FONT_FAMILY, FONT_SIZE
)
...
class MainWindow:
    ...
    def apply_filters(self):
        ...
        for row_idx, row in enumerate(products):
            (product_id, article, name, unit, price, supplier, manufacturer, 
             category, discount, stock_quantity, description, image_path) = row
            
            self.create_product_card(
                row_idx,
                product_id, article, name, description or "",
                category, manufacturer, supplier,
                price, unit, stock_quantity, discount,
                image_path
            )
    ...
    def open_add_product(self):
        ...
        app = AddProductWindow(add_win, self.user, self.resources_path, on_close=on_add_close)
    ...
    def open_edit_product(self, product_id):
        ...
        app = EditProductWindow(edit_win, self.user, product_id, self.resources_path, on_close=on_edit_close)
    ...
    def open_orders(self):
        ...
        app = OrdersWindow(orders_win, self.user, on_close=on_orders_close)
    ...
    def create_product_card(self, index, product_id, article, name, description,
                           category, manufacturer, supplier,
