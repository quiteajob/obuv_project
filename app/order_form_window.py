...
from database import get_order_details, add_order, update_order, get_unique_order_statuses, get_pickup_points, get_all_product_articles, get_article_by_display
...
class OrderFormWindow:
    def __init__(self, root, user, order_id=None, on_close=None):
        ...
        if user["role"] != "admin":
            messagebox.showerror("Ошибка доступа", "Только администратор может управлять заказами!", icon=messagebox.ERROR)
            root.destroy()
            return
        
        title = "Добавление заказа" if order_id is None else f"Редактирование заказа №{order_id}"
    ...
    def create_form(self):
        ...
        if self.order_id:
            ...
            if not order:
                messagebox.showerror("Ошибка", "Заказ не найден!", icon=messagebox.ERROR)
                self.root.destroy()
                return
            (_, order_date, delivery_date, status, pickup_point_id, pickup_address, items) = order
        else:
        ...
        # === Дата заказа ===
        self._create_field(form_frame, "order_date", "Дата заказа *:", width=20, default=order_date)
        
        # === Дата доставки ===
        self._create_field(form_frame, "delivery_date", "Дата доставки:", width=20, default=delivery_date or "")
        
        # === Статус (выпадающий список) ===
        tk.Label(form_frame, text="Статус заказа *:", font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(anchor="w", pady=(10, 5))
        
        statuses = get_unique_order_statuses()
        self.fields["status"] = ttk.Combobox(form_frame, values=statuses, font=(FONT_FAMILY, FONT_SIZE), width=30, state="readonly")
        ...
        tk.Label(form_frame, text="Пункт выдачи *:", font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(anchor="w", pady=(10, 5))
        
        pickup_points = get_pickup_points()
        self.pickup_points = pickup_points
        pickup_addresses = [addr for (_, addr) in pickup_points]
        
        self.fields["pickup_point"] = ttk.Combobox(form_frame, values=pickup_addresses, font=(FONT_FAMILY, FONT_SIZE), width=50, state="readonly")
        ...
        # === Товары заказа ===
        tk.Label(form_frame, text="Товары в заказе *:", font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(anchor="w", pady=(15, 5))
        ...
        self.items_listbox = tk.Listbox(list_frame, font=(FONT_FAMILY, FONT_SIZE - 1), height=8)
        ...
        tk.Button(btn_frame, text="➕ Добавить", font=(FONT_FAMILY, FONT_SIZE - 1), command=self.add_item).pack(fill=tk.X, pady=2)
        tk.Button(btn_frame, text="✏️ Изменить", font=(FONT_FAMILY, FONT_SIZE - 1), command=self.edit_item).pack(fill=tk.X, pady=2)
        tk.Button(btn_frame, text="🗑️ Удалить", font=(FONT_FAMILY, FONT_SIZE - 1), command=self.delete_item).pack(fill=tk.X, pady=2)
        
        self.items = items if items else []
        self._refresh_items_list()
        
        tk.Label(form_frame, text="💡 Выберите товар из списка и укажите количество", font=(FONT_FAMILY, FONT_SIZE - 2), bg=COLOR_MAIN_BG, fg="gray").pack(anchor="w", pady=(5, 0))
        
        # === Кнопки ===
        btn_frame = tk.Frame(form_frame, bg=COLOR_MAIN_BG)
        btn_frame.pack(pady=20)
        
        tk.Button(btn_frame, text="💾 Сохранить", font=(FONT_FAMILY, FONT_SIZE, "bold"), bg="#4CAF50", fg="white", command=self.save_order).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Отмена", font=(FONT_FAMILY, FONT_SIZE), command=self.on_closing).pack(side=tk.LEFT, padx=10)
    
    def _create_field(self, parent, field_name, label_text, width=20, default=""):
        """Создать поле ввода"""
        tk.Label(parent, text=label_text, font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(anchor="w", pady=(10, 5))
    ...
    def add_item(self):
        ...
        if not self.product_articles:
            messagebox.showerror("Ошибка", "В базе нет товаров! Сначала добавьте товары.", icon=messagebox.ERROR)
            return
        ...
        tk.Label(dialog, text="Выберите товар *:", font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(pady=(10, 5))
        
        # Выпадающий список с поиском
        article_combo = ttk.Combobox(dialog, values=self.product_articles, font=(FONT_FAMILY, FONT_SIZE), width=55, state="readonly")
        article_combo.pack(pady=5)
        article_combo.current(0)
        
        tk.Label(dialog, text="Количество *:", font=(FONT_FAMILY, FONT_SIZE), bg=COLOR_MAIN_BG).pack(pady=(10, 5))
        ...
        def on_confirm():
            ...
            try:
                ...
                messagebox.showerror("Ошибка", "Количество должно быть положительным числом!", parent=dialog)
                return
        ...
        tk.Button(dialog, text="Добавить", font=(FONT_FAMILY, FONT_SIZE), bg="#4CAF50", fg="white", command=on_confirm).pack(pady=10)
        tk.Button(dialog, text="Отмена", font=(FONT_FAMILY, FONT_SIZE), command=dialog.destroy).pack()
    ...
    def edit_item(self):
        ...
        tk.Button(dialog, text="Сохранить", font=(FONT_FAMILY, FONT_SIZE), bg="#4CAF50", fg="white", command=on_confirm).pack(pady=10)
        tk.Button(dialog, text="Отмена", font=(FONT_FAMILY, FONT_SIZE), command=dialog.destroy).pack()
    ...
    def validate_form(self):
        ...
        # Пункт выдачи
        if not self.fields.get("pickup_point") or not self.fields["pickup_point"].get().strip():
    ...
    def save_order(self):
        ...
        try:
            if self.order_id:
                ...
                messagebox.showinfo("Успех", f"Заказ №{self.order_id} обновлён!", icon=messagebox.INFO)
            else:
                add_order(data)
